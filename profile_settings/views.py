...
class MyProfileView(View):
    def get(self, request):
        ...
        return render(
            request,
            "profile_settings/my_profile.html",
...
class ChangePasswordView(View):
    def get(self, request):
        return render(
            ...
            context={"change_password": PasswordChangeForm(request.user)},
        )
...
def editTagsView(request):
    ...
    if is_ajax:
        if request.method == "POST":
            ...
            extraData.save()
            messages.success(
                request, "Änderungen wurden angenommen"
            )  # --> TODO: This is not working correctly as it shows the message on the password page ???
        ...
        return render(
            request,
            "profile_settings/teacher_change_tags.html",
            context={
                "tags": request.user.teacherextradata.tags.all().order_by("name"),
                "all_tags": list(
                    set(Tag.objects.all()).difference(
                        request.user.teacherextradata.tags.all().order_by("name")
                    )
                ),
            },
        )
